...
def generate_sa_models(base_package: types.ModuleType, dsn: str, schemas: str | list):
    ...
    generator = DeclarativeGenerator(base_package=base_package, metadata=_metadata, bind=engine,
                                     options={'nojoined', 'nobidi'})
    ...
    for full_name, table in tables.items():
        ...
        wrapped = textwrap.dedent(f"""
            ...
            class {table.name.title().replace('_', '')}(DBModel):
                ...
                {f"__model__ = {__model__}" if __model__ else ""}

                t: ClassVar[{tbl_type!r}]
                c: ClassVar[{column_class!r}]
                {f"m: ClassVar[{__model__}] = {__model__}" if __model__ else ""}
