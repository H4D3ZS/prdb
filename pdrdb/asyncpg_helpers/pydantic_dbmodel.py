...
class DBModel(BaseModel):
    ...
    def insert(self: TSelf, exclude_unset=True, returning=ALL_COLUMNS) -> 'WrappedInsert[TSelf]':
        stmt = WrappedInsert(
            ...
            **dbable_encoder(self.dict(exclude_unset=exclude_unset))
        ).returning(returning)
        return stmt
    ...
    def select_all(cls: Type[T]) -> 'WrappedSelect[T, asyncpg.Record | None]':
        stmt = WrappedSelect.create(cls.__table__, _model=cls).select_from(cls.__table__)
        return stmt

    @classmethod
    def update_stmt(cls: Type[T], **values) -> 'WreppedUpdate[T, asyncpg.Record | None]':
        serialized = dbable_encoder(values)
        stmt = WrappedUpdate(cls.__table__, values=serialized).returning(ALL_COLUMNS)
        return stmt
...
class MyWrappedQuery(Generic[BoundModelType, BoundRecordType]):
    ...
    async def fetch(self) -> List[FetchRetType]:
        res = await self._bound_model.fetch(self) if self._proxy_rows else await get_db().fetch(self)
        if self._as_model and res:
            return [self._bound_model(**row) for row in res]
        return res

    async def fetchrow(self) -> FetchRetType:
        res = await self._bound_model.fetchrow(self) if self._proxy_rows else await get_db().fetchrow(self)
        if self._as_model and res is not None:
            return self._bound_model(**res)
        return res

    async def fetchval(self):
        return await self._bound_model.fetchval(self) if self._proxy_rows else await get_db().fetchval(self)
    ...
    def as_proxy(self: TSelf) -> TSelf:
        self._proxy_rows = True
        return self
